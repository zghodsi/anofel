...
static bool test_jubjub_hash_varbits(ProtoboardT& pb, const char *name, const VariableArrayT& data_variables, const EdwardsPoint& expected)
{	
	const Params params;

	PedersenHash the_gadget(pb, params, name, data_variables, "the_gadget");
...
static bool test_jubjub_hash_vararray()
{	
	const char *name="pedersen_hash";
	ProtoboardT pb;
	// Testcases for vararrays
        VariableArrayT mpk_x;
	mpk_x.allocate(pb, 256, "mpk_x");
	mpk_x.fill_with_bits_of_field_element(pb, 
			FieldT("13866189866966177725545955315473288366756005698510089107323854294421070766415"));
	VariableArrayT mpk_y;
	mpk_y.allocate(pb, 256, "mpk_y");
	mpk_y.fill_with_bits_of_field_element(pb, 
	...
	EdwardsPoint expected={FieldT("12259485525025036854429235328638873705738271974558559157699957766968151069058"),
			FieldT("5667793519437459476959575145302880402694887467082943091682059295889357063990")};
	const VariableArrayT data_variables(flatten({mpk_x, mpk_y, D, salt}));
	return test_jubjub_hash_varbits(pb, name, data_variables, expected);
}


static bool test_jubjub_hash_bytes(const char *name, const uint8_t *data, size_t data_sz, const EdwardsPoint& expected)
{	
	ProtoboardT pb;
...
static bool test_jubjub_hash_bits(const char *name, const char *bits_str, const EdwardsPoint& expected)
{	
	ProtoboardT pb;
...
bool testcases_jubjub_hash( void )
{
	...
	const std::vector<TestcaseJubjubHash> testcases_bits = {
		...
		{"EdDSA_Verify.RAM", "110000011001000100111001110010111000010011000110100000001010011011100001010000010001011011011010000001100000010101100001011101001010000011001111101000010010000111100110111001011100001001000", {
			FieldT("14045508791325353580494855977548389716424041263427327046036601485042042851718"),
			FieldT("15954708971572753815495876887310661753138650367206890755040363216818746514839")}
		},
